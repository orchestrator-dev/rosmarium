# Cortex CMS: Kubernetes Deployment

Cortex CMS is designed to be cloud-native and deployable on Kubernetes via Helm.

## Architecture

The Helm chart (`deploy/helm/cortex`) provisions:
- **Cortex Server**: The primary Fastify API and GraphQL endpoint (`cortex-server`).
- **Cortex AI Worker**: The asynchronous background worker for ML/AI tasks (`cortex-ai-worker`).
...
```bash
helm install cortex ./deploy/helm/cortex -f values-prod.yaml -n cortex-cms --create-namespace
```
