...
## TypeScript (rosmarium-server & rosmarium-admin)
...
- X-Worker-Secret header validates internal callers (rosmarium-server → ai-worker)
- Tests: pytest + pytest-asyncio, asyncio_mode=auto; testcontainers for DB/Redis integration

## Database (Drizzle ORM)
- Schema in `apps/rosmarium-server/src/db/schema/`
- Migrations hand-written SQL in `apps/rosmarium-server/src/db/migrations/` (named `NNNN_<slug>.sql`)
- `pnpm db:generate` for drizzle-managed migrations; hand-written migrations copied manually
- pgvector embeddings stored in per-content-type tables: `rosmarium_{contentType}_embeddings`
...
## Error handling
- rosmarium-server → ai-worker: use custom error classes (e.g. `SearchEmbeddingError`), never let raw network errors bubble up; graceful fallback to fulltext-only search when AI worker unreachable
