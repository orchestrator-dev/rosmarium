...
### Changed
- **Monorepo Version Bump**: Synchronized all 12 Node.js apps/packages and Python AI worker to version `3.0.1`.
- **Release Verification & CI Pipeline**: Resolved TypeScript linting errors across `@orchestrator.dev/server`, `@orchestrator.dev/admin`, and SDK packages. Configured warning rules for legacy modules to ensure clean builds.
- **Vitest Unit Testing**: Added `jsdom` test environment support to `@orchestrator.dev/admin` and implemented `afterEach(cleanup)` to prevent DOM test leakage.
