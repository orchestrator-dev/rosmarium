# Contributing to Cortex CMS

Welcome to Cortex CMS! We're building a modern, extensible AI-first content management system and we are thrilled you're interested in contributing.
...
**Step by step:**
```bash
git clone https://github.com/orchestrator-dev/cortex.git
cd cortex
pnpm install
cp .env.example .env
pnpm infra:up
...
- **cortex-server**: `pnpm --filter @cortex-cms/server dev`
- **cortex-ai-worker**: `pnpm worker:dev`

**Running tests:**
- Server: `pnpm --filter @cortex-cms/server test`
...
|---|---|---|
| `cortex-server` | CMS API, GraphQL, Auth, Assets | TypeScript |
| `cortex-admin` | Admin UI | TypeScript/React |
| `cortex-ai-worker` | AI pipeline, embeddings, RAG | Python |
| `cortex-cli` | Developer CLI | Go |
