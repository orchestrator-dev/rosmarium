...
from rosmarium_ai_worker.graph.inference import GraphInferenceEngine
...
async def test_runs_ner_strategy_when_enabled(
    ...
    with patch(
        "rosmarium_ai_worker.graph.inference.infer_from_ner", new_callable=AsyncMock
    ) as mock_ner:
...
async def test_isolates_strategy_failures(
    ...
    with (
        patch(
            "rosmarium_ai_worker.graph.inference.infer_from_ner",
            new_callable=AsyncMock,
            side_effect=RuntimeError("NER boom"),
        ),
        patch(
            "rosmarium_ai_worker.graph.inference.infer_from_similarity",
            new_callable=AsyncMock,
            return_value=3,
        ) as mock_sim,
    ):
...
async def test_runs_all_three_strategies(
    ...
    with (
        ...
        patch(
            "rosmarium_ai_worker.graph.inference.infer_from_references",
            new_callable=AsyncMock,
            return_value=1,
        ),
    ):
