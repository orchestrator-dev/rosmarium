...
class KnowledgeGraphExporter:
    async def export_json_ld(
        ...
        if not entry_ids:
            return {
                "@context": {
                    ...
                    "cortex": "https://cortex-cms.dev/vocab#",
                    "relatedTo": "cortex:relatedTo",
                    "references": "cortex:references",
                    "mentions": "cortex:mentions",
                },
                "@graph": [],
            }
        ...
        for edge in edges:
            ...
            rel_type = "cortex:relatedTo"
            if edge["edge_type"] == "reference":
                rel_type = "cortex:references"
            elif edge["edge_type"] == "mention":
                rel_type = "cortex:mentions"
                
            relations[from_id].append({
                "predicate": rel_type,
                "target": f"cortex:entry/{to_id}"
            })
    ...
    async def export_rdf_turtle(
        ...
        if not json_ld_data["@graph"]:
            return "@prefix cortex: <https://cortex-cms.dev/vocab#> .\n@prefix schema: <https://schema.org/> .\n"
