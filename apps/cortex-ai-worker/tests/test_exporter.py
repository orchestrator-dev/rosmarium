...
from cortex_ai_worker.graph.exporter import KnowledgeGraphExporter
...
async def test_export_json_ld(exporter: KnowledgeGraphExporter, mock_conn: MagicMock) -> None:
    ...
    node_a = next(n for n in data["@graph"] if n["@id"] == "cortex:entry/A")
    assert node_a["@type"] == "schema:Article"
    assert node_a["cortex:relatedTo"][0]["@id"] == "cortex:entry/B"
