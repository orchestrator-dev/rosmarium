 ...
 * Queries information_schema to find tables matching the cortex_*_embeddings pattern.
 */
async function resolveContentTypes(requested?: string[]): Promise<string[]> {
    ...
    try {
        ...
        const rows = await client.execute(
            `SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE 'cortex_%_embeddings'`
        );
        if (Array.isArray(rows) && rows.length > 0) {
            return rows.map((r) =>
                (r.table_name ?? "").replace(/^cortex_/, "").replace(/_embeddings$/, "")
            );
        }
    } catch {
